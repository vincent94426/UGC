 ...
 * @author Rosson Chen
 */
public final class VideoCaptureActivity extends Activity implements SurfaceHolder.Callback,UGCSettingPreference{
	...
	static final String TAG	= "VideoCaptureActivity" ;
	
	private static final String ParentPath = Environment.getExternalStorageDirectory().getAbsolutePath() + UgV2Constants.CONTENT_PATH_VIDEO;
	...
	private final boolean loadRecorderParameter(){
		...
		mWidth	= SettingUtil.readInt(getApplicationContext(), UgV2Constants.KEY_VIDEO_SELECTED_WIDTH, DEFAULT_WIDTH);
		mHeight	= SettingUtil.readInt(getApplicationContext(), UgV2Constants.KEY_VIDEO_SELECTED_HEIGHT, DEFAULT_HEIGHT);
		
		int quality = SettingUtil.readInt(getApplicationContext(), UgV2Constants.KEY_VIDEO_SELECTED_QUALITY, UgV2Constants.VIDEO_QUALITY_LOW);
