...
public final class DebugUtil {
	...
	private static PrintWriter getDebugLogWriter()
		...
		String rootPath = Environment.getExternalStorageDirectory()
				.getAbsolutePath();
		File logDir = new File(rootPath + File.separator
				+ NpmConstants.PRIVATE_DATA_PATH);
		if (!TextUtils.isEmpty(rootPath)
				&& (logDir.exists() || logDir.mkdirs())) {
			FileOutputStream fos = null;
			File logFile = new File(rootPath + File.separator
					+ NpmConstants.PRIVATE_DATA_PATH + "trace.txt");
			// 日志文件大于2M后，覆盖重写
			...
			return new PrintWriter(new OutputStreamWriter(fos,
					NpmConstants.DEFAULT_ENCODEING), false);
		}
		return null;
	}
}